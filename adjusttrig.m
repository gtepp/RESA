...
% NOTE: For "INDEX" option, added alignment by tracenum which was
% previously missing. (Updated 3/20/17, G. Tepp)
...
if length(varargin)>=1 && isa(varargin{1},'double')      % use shifttriggers
    ...
    if strncmp(calctype,'IND',3) && (length(varargin)==2)
        dosubset = varargin{2};
    else
        dosubset = 0;
    end;
...
if (index~=0) && strcmpi(calctype(1:3),'MIN')
